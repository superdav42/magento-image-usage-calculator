...
namespace DevStone\UsageCalculator\Model\Usage\Option\Type;
...
class Date extends \Magento\Catalog\Model\Product\Option\Type\DefaultType
{
    ...
    public function validateUserValue($values)
    {
        ...
        if ($this->_dateExists()) {
            if ($this->useCalendar()) {
                $dateValid = isset($value['date']) && preg_match('/^\d{1,4}.+\d{1,4}.+\d{1,4}$/', (string) $value['date']);
            } else {
                $dateValid = isset(
                    $value['day']
                ) && isset(
        ...
        if ($this->getIsValid() && $this->getUserValue() !== null) {
            ...
            if ($this->_timeExists()) {
                ...
                if (!$this->is24hTimeFormat()) {
                    $pmDayPart = 'pm' == strtolower((string) $value['day_part']);
                    if (12 == $value['hour']) {
